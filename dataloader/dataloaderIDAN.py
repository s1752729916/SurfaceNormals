...
class IDANSurfaceDataset(Dataset):
    """
    ...
    def __init__(self,  label_dir,input_I_sum_dir,dolp_dir,aolp_dir,synthesis_normals_dir,mask_dir=None, transform=None, input_only=None):
    ...
    def __getitem__(self, index):
        ...
        input_img_arr = numpy.zeros([15, height, width], dtype=np.uint8)  # shape is (13 x H x W)
        input_img_arr[0, :, :] = I_sum_path
        input_img_arr[1,:,:] = dolpImg
        input_img_arr[2,:,:] = aolpImg
        input_img_arr[3:6, :, :] = norm_0.transpose(2, 0, 1)  # 3 x H x W
        input_img_arr[6:9, :, :] = norm_1.transpose(2, 0, 1)
        input_img_arr[9:12, :, :] = norm_2.transpose(2, 0, 1)
        input_img_arr[12:15, :, :] = norm_3.transpose(2, 0, 1)
