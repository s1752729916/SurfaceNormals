...
class ISurfaceDataset(Dataset):
    ...
    def __init__(self,  label_dir,input_I_sum_dir,mask_dir=None, transform=None, input_only=None):
    ...
    def __getitem__(self, index):
        ...
        input_img_arr = numpy.zeros([1, height, width], dtype=np.uint8)  # shape is (3 x H x W)
    ...
    def _create_lists_filenames(self):
        ...
        # -- 5、verify number of every input
            if not (numISumImages== numMasks == numLabels):
                print(numISumImages,numMasks, numLabels)
                raise ValueError('Numbers of inputs(I_sum,mask,label) are different.')
