...
class SfPSurfaceDataset(Dataset):
    ...
    def __init__(self,  label_dir,input_poar_dir,synthesis_normals_dir,mask_dir=None, transform=None, input_only=None):
        self.input_polar_dir = input_poar_dir
    ...
    def __getitem__(self, index):
        ...
        input_img_arr = numpy.zeros([16, height, width], dtype=np.uint8)  # shape is (13 x H x W)
        input_img_arr[0,:,:] = I_0
        input_img_arr[1,:,:] = I_45
        input_img_arr[2,:,:] = I_90
        input_img_arr[3,:,:] = I_135
        input_img_arr[4:7, :, :] = norm_0.transpose(2, 0, 1)  # 3 x H x W
        input_img_arr[7:10, :, :] = norm_1.transpose(2, 0, 1)
        input_img_arr[10:13, :, :] = norm_2.transpose(2, 0, 1)
        input_img_arr[13:16, :, :] = norm_3.transpose(2, 0, 1)
        ...
        a = torch.split(input_tensor,[4,12],dim=0)
        polar_tensor = a[0]
        normal_tensor = a[1]
        if(self.masks_dir is not None):
            return polar_tensor,normal_tensor, label_tensor, mask_tensor
        else:
            return polar_tensor,normal_tensor,label_tensor
    ...
    def _create_lists_filenames(self):
        ...
        '''
        assert os.path.isdir(self.input_polar_dir), 'Dataloader given rgb I_sum_dir directory that does not exist: "%s"' % (self.input_polar_dir)
        ...
        # -- input polar images
        I_0_search_str = os.path.join(os.path.join(self.input_polar_dir,'I-0'), '*.png')
        self._datalist_polar_0 = sorted(glob.glob(I_0_search_str))
        ...
        I_45_search_str = os.path.join(os.path.join(self.input_polar_dir,'I-45'), '*.png')
        self._datalist_polar_45 = sorted(glob.glob(I_45_search_str))
        numISumImages = len(self._datalist_polar_45)
        if numISumImages == 0:
            raise ValueError('No rgb images found in given directory. Searched in dir: {} '.format(self.input_polar_dir))
        I_90_search_str = os.path.join(os.path.join(self.input_polar_dir,'I-90'), '*.png')
        self._datalist_polar_90 = sorted(glob.glob(I_90_search_str))
        numISumImages = len(self._datalist_polar_90)
        if numISumImages == 0:
            raise ValueError('No rgb images found in given directory. Searched in dir: {} '.format(self.input_polar_dir))
        I_135_search_str = os.path.join(os.path.join(self.input_polar_dir,'I-135'), '*.png')
        self._datalist_polar_135 = sorted(glob.glob(I_135_search_str))
