...
class ResNet(nn.Module):
    ...
    def __init__(self,in_channels, block, layers, output_stride, BatchNorm, pretrained=True):
        ...
        # Modules
        self.conv0 = nn.Conv2d(in_channels,64,kernel_size=1)
        self.bn0 = BatchNorm(64)
        self.conv1 = nn.Conv2d(64, 64, kernel_size=7, stride=2, padding=3,   # 这里修改输入的通道数
                                bias=False)
...
def ResNet101(output_stride, BatchNorm, pretrained=True):
    ...
    return model
def ResNet50(in_channels,output_stride, BatchNorm, pretrained=True,Fusion = False):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    if(Fusion):
        model = FusionResNet(in_channels = in_channels,block=EPSABlock, layers = [3, 4, 6, 3], output_stride=output_stride, BatchNorm = BatchNorm, pretrained=pretrained)
    else:
