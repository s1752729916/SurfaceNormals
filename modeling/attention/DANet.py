...
from modeling.attention.SimplifiedSelfAttention import SimplifiedScaledDotProductAttention
from modeling.attention.ExternalAttention import ExternalAttention
...
class PositionAttentionModule(nn.Module):
    ...
    def __init__(self, d_model=512, kernel_size=3, H=7, W=7):
        ...
        self.pa = ExternalAttention(d_model,S = 16)
    def forward(self, x):
        bs, c, h, w = x.shape
        y = self.cnn(x)
        y = y.view(bs, c, -1).permute(0, 2, 1)  # bs,h*w,c
        y = self.pa(y)  # bs,h*w,c
        return y
