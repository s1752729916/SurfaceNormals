...
class smqFusion(nn.Module):
    def __init__(self, backbone='resnet', output_stride=16, num_classes=21,
        ...
        # branch
        self.backbone_orig = build_backbone(in_channels=2 + 3,backbone=backbone, output_stride=output_stride, BatchNorm=BatchNorm,Fusion=True)
        self.aspp_orig = build_aspp(backbone, output_stride, BatchNorm)
        self.backbone_prior = build_backbone(in_channels=12 + 3,backbone=backbone, output_stride=output_stride, BatchNorm=BatchNorm,Fusion=True)
        ...
        self.backbone_atten = build_backbone(in_channels=1 + 3,backbone=backbone,output_stride=output_stride,BatchNorm=BatchNorm,Fusion=True)
    ...
    def forward(self, orig,prior):
        ...
        bs,c,h,w = orig.shape
        if(bs>1):
            orig = torch.concat((orig, self.V),dim=1)
            prior = torch.concat((prior,self.V),dim=1)
            atten_map = torch.concat((atten_map,self.V),dim=1)
        else:
            orig = torch.concat((orig, self.V1), dim=1)
            prior = torch.concat((prior, self.V1), dim=1)
            atten_map = torch.concat((atten_map, self.V1), dim=1)
        ...
        # x_prior_0 = x_atten_0 + self.attention_layer(x_atten_0,x_atten_0,x_prior_0)
        # x_prior_1 = self.attention_layer(x_atten_1,x_atten_1,x_prior_1)
        # x_prior_2 = self.attention_layer(x_atten_2,x_atten_2,x_prior_2)
        # x_prior_3 = self.attention_layer(x_atten_3,x_atten_3,x_prior_3)
        # x_prior_4 = self.attention_layer(x_atten_4,x_atten_4,x_prior_4)
        ...
        # x_fusion = 0.5*(x_orig + x_prior)
        # x_fusion_0 = 0.5*(x_orig_0 + x_prior_0)
        # x_fusion_1 = 0.5*(x_orig_1 + x_prior_1)
        # x_fusion_2 = 0.5*(x_orig_2 + x_prior_2)
        # x_fusion_3 = 0.5*(x_orig_3 + x_prior_3)
        # x_fusion_4 = 0.5*(x_orig_4 + x_prior_4)
