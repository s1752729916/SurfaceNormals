...
class smqNet(nn.Module):
    ...
    def forward(self, orig,prior):
        ...
        # x_atten = torch.cat((normal_orig,normal_prior,atten_map),dim = 1)
        ...
        x_fusion = x_atten*x_prior + x_orig
        low_level_feat_fusion = low_level_feat_atten*low_level_feat_prior + low_level_feat_orig
        # x_fusion = 0.5*x_orig +0.5*x_prior
        # low_level_feat_fusion = 0.5*low_level_feat_orig + 0.5*low_level_feat_prior

        # x_fusion = torch.cat((x_orig,x_prior,x_atten),dim = 1)
        # low_level_feat_fusion = torch.cat((low_level_feat_orig,low_level_feat_prior,low_level_feat_atten),dim = 1)
        # x_atten,low_level_feat_atten = self.backbone_atten(x_atten)
        # x_atten = self.aspp_atten(x_atten)
        input = torch.cat((orig,prior),dim=1)
        x = self.decoder_atten(x_fusion,low_level_feat_fusion,input)
