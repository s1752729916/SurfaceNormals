...
from modeling.UNet.unet_model import UNet
from modeling.deepSfp.deepSfp import deepSfp
from modeling.DeepLab.DeepLab import DeepLab
from modeling.Kondo.Kondo import Kondo
from modeling.SPW.SPW import SPW
...
# model = UNet(n_channels=2+12+1,n_classes=3,bilinear=True,device=device)
# model = DeepLab(in_channels=2+12+1,num_classes=3,backbone='resnet50',device=device)
# model = PS_FCN(device=device)
# model = SPW(n_channels=2+12+1,n_classes=3,bilinear=True,device=device)
# model = Kondo(c_in=2,device=None)
model = deepSfp(input_channel=2+12+1,device=device)
...
for epoch in range(START_EPOCH,MAX_EPOCH):
    ...
    for iter_num,batch  in enumerate(tqdm(trainLoader)):
        ...
        if(epoch % 10==0):
            ...
            API.utils.png_saver(
                os.path.join('/home/robotlab/smq/SurfaceNormals/comparison/results/train', str(iter_num).zfill(3) + '-label.png'),
                label_t_rgb)
            API.utils.png_saver(
                os.path.join('/home/robotlab/smq/SurfaceNormals/comparison/results/train', str(iter_num).zfill(3) + '-predict.png'),
                predict_norm_rgb)
            API.utils.png_saver(
                os.path.join('/home/robotlab/smq/SurfaceNormals/comparison/results/train', str(iter_num).zfill(3) + '-atten.png'),
                atten_map_rgb)
